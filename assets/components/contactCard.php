...
<?php
// Expects: $c (array) and helpers e(), formatCreated(), renderTags()
...
<article class="contact-card<?php echo $isFav ? ' is-favorited' : ''; ?>" data-contact-id="<?php echo $id; ?>">
  ...
  <div class="cc-actions-top">
    <!-- Favorite toggle -->
    <form class="cc-fav-form" method="post" action="/contacts/toggle-favorite.php"
          data-endpoint="/contacts/toggle-favorite.php" data-contact-id="<?php echo $id; ?>">
      <input type="hidden" name="id" value="<?php echo $id; ?>">
      <input type="hidden" name="favorited" value="<?php echo $isFav ? '0' : '1'; ?>">
      <button class="cc-icon cc-favorite<?php echo $isFav ? ' active' : ''; ?>"
        ...
        <svg viewBox="0 0 28 28" width="24" height="24" aria-hidden="true">
          <path
            d="M6.8 24.5L8.7 16.3 2.33 10.79 10.73 10.06 14 2.33 17.27 10.06 25.67 10.79 19.31 16.3 21.2 24.5 14 20.15 6.8 24.5Z"
            fill="<?php echo $isFav ? 'var(--accent)' : 'none'; ?>"
  ...
  <div class="cc-actions-bottom">
    <!-- Delete -->
    <form class="cc-delete-form" method="post" action="/contacts/delete.php"
          data-endpoint="/contacts/delete.php" data-contact-id="<?php echo $id; ?>">
      <input type="hidden" name="id" value="<?php echo $id; ?>">
      <button class="cc-icon cc-delete" aria-label="Delete" title="Delete" data-action="delete">
        <svg viewBox="0 0 28 28" width="24" height="24" aria-hidden="true">
